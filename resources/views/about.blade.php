@extends('layouts.master')
@section('content')

    <div class="container">
        <div class="row">

            <div class="col-sm-offset-1 col-sm-10 content" style="margin-bottom: 30px;">
                <div class="row">

                    <div class="col-sm-5">
                        <img class="img-responsive" src="{{ url('img/dobrotex.jpg') }}" style="margin-bottom: 20px; border: 1px solid #000">
                    </div>
                    <div class="col-sm-7 Philosopher font-size-about justify" style="padding-top: 35px;">
                        Dobrotex – это молодая украинская развивающаяся компания, которая смогла всего за 7 лет положительно зарекомендовать себя на рынке пухо-перьевых изделий.
                    </div>
                    <div class="col-sm-12 Philosopher font-size-about justify">
                        Нашей задачей является обеспечить Вам комфортный и качественный сон, потому для изготовления подушек и одеял мы используем исключительно перо и пух наивысшего сорта, новейшее оборудование и тщательно следим за технологией производства.
                    </div>
                    <div class="col-sm-12 Philosopher font-size-about justify">
                        На данный момент, мы являемся одним из лидеров оптовых и розничных продаж пухо-перьевых изделий и смесей в Украине в бюджетном сегменте. Таким образом, за доступную цену Вы приобретаете не только отличный продукт, но и возможность ощутить на себе все преимущества глубокого, здорового и крепкого сна.
                    </div>

                </div>
            </div>

            <div class="col-sm-offset-1 col-sm-10 content">
                <div class="row">

                    <div class="col-sm-12 Philosopher" style="font-size: 16px;">
                        <h3> Как нас найти ? </h3>
                        <hr>
                        <div class="col-sm-7">
                            <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2513.989565368562!2d34.79010943462935!3d50.94240602056672!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x412903ba008af7f9%3A0x191aa7eca89c7616!2z0LLRg9C70LjRhtGPINCc0LDRiNC40L3QvtCx0YPQtNGW0LLQvdC40LrRltCyLCA0LCDQodGD0LzQuCwg0KHRg9C80YHRjNC60LAg0L7QsdC70LDRgdGC0Yw!5e0!3m2!1sru!2sua!4v1486930331794" width="100%" height="350px" frameborder="0" style="border:0" allowfullscreen></iframe>
                        </div>
                        <div class="col-sm-5">

                            <label> Адрес: </label><br>
                                Машиностроителей , 4, Украина , г.Сумы<br><br>
                            <label> График работы: </label><br>
                                ПН-СБ 9:00 - 17:00 <br><br>
                            <label> Контакты: </label><br>
                                066-06-222-06 (розница)<br>
                                066-06-333-06 (опт)


                        </div>
                    </div>

                </div>
            </div>

        </div>
    </div>

@endsection